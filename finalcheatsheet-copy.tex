...
\definition{Markov Bound} $\Pr[Z \geq a] \leq \frac{\E Z}{a}$

\definition{Chebychev Bound} $\Pr[|Z - \E Z| \geq \epsilon] \leq \frac{\text{Var}[Z]}{\epsilon^2}$

Recall, if $X = (Z-\E Z)^2$, $\text{Var}[Z] = \E[X]$.

\definition{Chernoff Bound} $\Pr[\sum y_i > (1 + \beta) \E [\sum y_i]] \leq e^{-\beta \E[\sum y_i] / 3}$

\definition{Chernoff Bound} $\Pr[\sum y_i < (1 - \beta) \E [\sum y_i]] \leq e^{-\beta \E[\sum y_i] / 2}$ 

\definition{SUHA} A hash function $h$ will evenly distribute items into the slots of a hash table. $h$ is independent of things already placed.

%------------------------------------

\section{Lecture 8: Greedy Algorithms and MST}

\definition{Kruskal's Algorithm} Pick the lowest-weight edge such that no cycles are formed at each step. runtime $E\log E$
